...
class KrakenAccountClient:
    ...
    def get_usdc_equity(self) -> float:
        try:
            ...
            # Sum USD stables to keep capital visible when account is not purely USDC.
            total = 0.0
            for key in ("USDC", "USDT", "ZUSD", "USD"):
                value = balances.get(key)
                if value is not None:
                    total += float(value)
            if total > 0:
                return total
            logging.warning("USD stable balance not found on Kraken. Keys: %s", ",".join(balances.keys()))
            return 0.0
        except Exception as exc:
